...
const updateProductInCart = catchAsync(async (req, res) => {
  ...
  let cart = await cartService.updateProductInCart(req.user, req.body.productId, req.body.quantity)
...
const checkout = catchAsync(async (req, res) => {
   let checkoutCart = await cartService.checkout(req.user);
  return (
    res.status(httpStatus.NO_CONTENT).send(checkoutCart)
  );
});
